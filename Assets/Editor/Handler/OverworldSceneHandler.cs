...
namespace Editor.Handler
{
    ...
    public static class OverworldSceneHandler
    {
        ...
        private static void CheckSceneState()
        {
            ...
            if (!mainControl)
            {
                UCT.Debug.LogWarning("OverworldSceneHandler 在 MainControlSummon 物体上未找到脚本。");
                return;
            }
        ...
        public static void CreateLanguagePackFiles(string rootPath, string sceneName)
        {
            ...
            {
                UCT.Debug.LogError("场景初始化失败：rootPath参数不能为空");
                return;
            }

            if (string.IsNullOrEmpty(sceneName))
            {
                UCT.Debug.LogError("场景初始化失败：name参数不能为空");
                UCT.Debug.Log("你可能在尝试创建一个Overworld场景并运行，但尚未保存此场景。");
                UCT.Debug.Log("请先保存此场景，并为其命名，这个名称不能和其他场景相同。");
                UCT.Debug.Log("如果可以，把此场景添加进Scenes In Build，然后再次运行场景。场景将会自动进行初始化。");
                return;
            }

            if (!Directory.Exists(rootPath))
            {
                UCT.Debug.LogError($"场景初始化失败：{rootPath} 路径不存在");
                return;
            }

            if (sceneName.IndexOfAny(Path.GetInvalidFileNameChars()) >= 0)
            {
                UCT.Debug.LogError("场景初始化失败：场景名称包含非法文件名字符");
                return;
            }
            ...
            foreach (var subDir in Directory.GetDirectories(rootPath))
            {
                try
                {
                    ...
                    UCT.Debug.Log($"已生成{sceneName}场景的语言包文件: {filePath}");
                }
                catch (Exception ex)
                {
                    UCT.Debug.LogError($"处理目录 {subDir} 时出错: {ex.Message}");
                }
            }
        }
        ...
        public static void EnsureScriptableObjects(string path, string sceneName)
        {
            ...
            UCT.Debug.Log($"已生成{sceneName}场景的事件系统文件！");
            UCT.Debug.LogWarning("重新加载此场景后即可正常运行！");
