...
namespace TMPro.Examples
{
    public class TMP_TextSelector_B : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerClickHandler, IPointerUpHandler
    {
        ...
        private void LateUpdate()
        {
            if (isHoveringObject)
            {
                ...
                if (linkIndex != -1 && linkIndex != m_selectedLink)
                {
                    ...
                    // DebugLogger.Log("Link ID: \"" + linkInfo.GetLinkID() + "\"   Link Text: \"" + linkInfo.GetLinkText() + "\""); // Example of how to retrieve the Link ID and Link Text.
        ...
        {
            //DebugLogger.Log("OnPointerEnter()");
            isHoveringObject = true;
        }

        public void OnPointerExit(PointerEventData eventData)
        {
            //DebugLogger.Log("OnPointerExit()");
            isHoveringObject = false;
        }

        public void OnPointerClick(PointerEventData eventData)
        {
            //DebugLogger.Log("Click at POS: " + eventData.position + "  World POS: " + eventData.worldPosition);
            ...
            if (charIndex != -1 && charIndex != m_lastIndex)
            {
                //DebugLogger.Log("Character [" + m_TextMeshPro.textInfo.characterInfo[index].character + "] was selected at POS: " + eventData.position);
                m_lastIndex = charIndex;
            ...
            */
            ...
            if (linkIndex != -1)
            {
                ...
                //DebugLogger.Log(TMP_TextUtilities.GetSimpleHashCode("id_02"));
