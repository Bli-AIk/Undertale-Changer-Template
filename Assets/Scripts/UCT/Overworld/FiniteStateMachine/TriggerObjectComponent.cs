...
namespace UCT.Overworld.FiniteStateMachine
{
    public class TriggerObjectComponent : MonoBehaviour
    {
        ...
        private void Update()
        {
            if (!fsm || !_triggerCollider) return;

            var direction = fsm.data.directionPlayer;

            if (specifiedDirectionsList.Count == 0 || colliderOffsetsList.Count == 0) return;
            ...
            if (closestIndex >= 0) ApplyTriggerSettings(closestIndex);
        }
        ...
        private void ApplyTriggerSettings(int index)
        {
            ...
            if (index >= colliderOffsetsList.Count || index >= colliderSizesList.Count) return;
