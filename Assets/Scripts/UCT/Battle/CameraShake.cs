...
namespace UCT.Battle
{
    public class CameraShake : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (moveWithHeart && _tweenMoveBack == null && TurnController.Instance.isMyTurn && transform.position != Vector3.zero)
            {
        ...
        public void Shake(Vector3 v3Move, Vector3 v3Spin, int loops = 4, float shakeTime = 1f / 60f * 4f, string getSon = "", Ease easeMove = Ease.OutCubic, Ease easeSpin = Ease.InOutCubic)
        {
            ...
            {
                _tweenMove = transformer.DOLocalMove(transformer.localPosition + v3Move, shakeTime).SetLoops(loops, LoopType.Yoyo).SetEase(easeMove);
            }

            if (v3Spin != Vector3.zero)
            {
                _tweenSpin = transformer.DOLocalRotate(transformer.localRotation.eulerAngles + v3Spin, shakeTime).SetLoops(loops, LoopType.Yoyo).SetEase(easeSpin);
            }
