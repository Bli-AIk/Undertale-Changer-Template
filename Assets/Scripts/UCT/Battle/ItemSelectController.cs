...
namespace UCT.Battle
{
    ...
    public class ItemSelectController : MonoBehaviour
    {
        ...
        public void Open()
        {
            for (var i = 2; i < sons.Count; i++) sons[i].gameObject.SetActive(i <= myItemMax + 1);
            ...
            sons[0].transform.DOLocalMoveY(sons[0].transform.localPosition.y + 0.05f, 0.75f).SetLoops(-1, LoopType.Yoyo)
                .SetEase(Ease.InOutQuad);
            sons[1].transform.DOLocalMoveY(sons[1].transform.localPosition.y - 0.05f, 0.75f).SetLoops(-1, LoopType.Yoyo)
                .SetEase(Ease.InOutQuad);
            PressDown(true);
        }
        ...
        public void PressDown(bool isUp)
        {
            ...
            _tweenSave = DOTween
                .To(() => sonsChanged[myItemSelect].transform.localScale,
                    x => sonsChanged[myItemSelect].transform.localScale = x, Vector3.one * 3, 0.5f)
